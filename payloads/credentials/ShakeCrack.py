...
from file_browser import browse_file  # reuse module
...
def draw_ui():
    ...
    draw.text((4,3), "HANDSHAKE CRACKER", font=font_sm, fill="#FF3333")
    draw.text((4,20), f"File: {os.path.basename(selected_cap) if selected_cap else 'None'}", font=font_sm, fill="#FFBBBB")
    draw.text((4,32), f"Status: {status}", font=font_sm, fill="#FFBBBB")
    draw.text((4,44), f"Result: {result[:20]}", font=font_sm, fill="#88FF88" if "KEY" in result else "#FFBBBB")
    draw.rectangle((0,H-12,W,H), fill="#220000")
    draw.text((4,H-11), "K1=Select  K2=Crack  K3=Exit", font=font_sm, fill="#FF7777")
    push()

def select_file():
    global selected_cap, result, status
    cap = browse_file("/home/kali", [".cap", ".pcap"])
    if cap:
        selected_cap = cap
        result = ""
        status = "File loaded"
...
def run_crack():
    ...
    if not selected_cap or not os.path.exists(selected_cap):
    ...
    draw_ui()
    # Run aircrack-ng
    cmd = ["aircrack-ng", "-w", wordlist, selected_cap]
