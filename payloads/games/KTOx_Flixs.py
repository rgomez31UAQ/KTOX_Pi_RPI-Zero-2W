...
<style>
    ...
    input[type="text"], input[type="file"] { 
        ...
        padding: 10px; width: 100%; margin-bottom: 15px; font-family: inherit;
    }
    .btn { 
    ...
    #prog-wrap { display: none; margin-top: 20px; border: 1px solid var(--cyan); height: 20px; position: relative; }
    #prog-bar { width: 0%; background: var(--cyan); height: 100%; transition: width 0.1s; }
    #prog-text { position: absolute; width: 100%; text-align: center; font-size: 10px; color: white; top: 2px; mix-blend-mode: difference; }
    .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 15px; padding: 20px; }
    .card { background: #000; border: 1px solid #2b0000; text-decoration: none; color: inherit; }
...
def uplink_home():
    ...
    <div class="container">
        <label style="font-size:10px;">TARGET_PATH</label>
        <input type="text" id="sd" placeholder="Optional: Season 1 / Movies / etc">
        
        <label style="font-size:10px;">DATA_SOURCE (FILES OR FOLDERS)</label>
    ...
    function up(){
        ...
        if(files.length === 0) return alert("Select data first.");
    ...
    try:
        while True:
            ...
            if show_qr:
                ...
                draw.text((4,3), "CYBER_VOID NODE", fill="black")
