...
Features:
- Auto-detect wireless interface (wlan0/wlan1)
- Browse any wordlist from any directory
...
def find_wireless_interface():
    """Return first wlan interface (wlan0, wlan1, etc.) that exists."""
...
def enable_monitor(iface):
    """Enable monitor mode on given interface, return monitor interface name."""
    ...
    mon = f"{iface}mon"
    # Sometimes iw doesn't create mon, use airmon-ng as fallback
    ...
    if os.path.exists(f"/sys/class/net/{mon}"):
        return mon
    # If still no, try using the original interface as monitor
    return iface

def disable_monitor(iface):
    """Disable monitor mode and restore managed mode."""
    mon = f"{iface}mon"
    run_cmd(f"airmon-ng stop {mon} 2>/dev/null")
    run_cmd(f"ip link set {iface} down")
...
def scan_targets(mon):
    draw_screen(["Scanning targets...", "Please wait ~15s", f"Interface: {mon}"])
...
def main():
    ...
    if not iface:
        draw_screen(["No wireless interface", "found (wlan0/wlan1)", "KEY3 to exit"], text_color="#FF4444")
    ...
    mon = None
    try:
        draw_screen([f"Using {iface}", "Enabling monitor mode..."])
        mon = enable_monitor(iface)
        if not mon:
            draw_screen(["Monitor mode failed", "Check airmon-ng", "KEY3 to exit"], text_color="#FF4444")
            while wait_button(0.5) != "KEY3":
                pass
            return
        ...
        if not targets:
            draw_screen(["No APs found", "Try different location", "KEY3 to exit"], text_color="#FF8888")
            while wait_button(0.5) != "KEY3":
                pass
            return
