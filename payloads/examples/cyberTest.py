    ...
    while any(e.is_alive() for e in enemies) and any(p.is_alive() for p in player_crew):
        ...
        if fighter in player_crew:
            ...
            if action == -1:
                return "hub"
            if action == 0:
                target_idx = menu_choice([f"{e.name} HP:{e.hp}/{e.max_hp}" for e in enemies if e.is_alive()], title="Attack whom?")
                if target_idx == -1:
                    return "hub"
                target = [e for e in enemies if e.is_alive()][target_idx]
                ...
                if hack_choice == 0 and game.energy >= 20:
                    game.energy -= 20
                    dmg = 20
                    target = [e for e in enemies if e.is_alive()][0]
                    actual = target.take_damage(dmg)
            ...
            # Enemy AI
            target = random.choice([p for p in player_crew if p.is_alive()])
